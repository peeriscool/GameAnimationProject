...
quat conjugate(const quat& q)  //The inverse of a normalized quaternion.
{
	return quat(
		-q.x,
		-q.y,
...
quat inverse(const quat& q) //conjugate divided by the squared length of the quaternion.
{
	float lenSq = q.x * q.x + q.y * q.y + q.z * q.z + q.w * q.w;
	if (lenSq < QUAT_EPSILON) {
		return quat();
...
//The squared length of a normalized quaternion is always 1.
#if 1
