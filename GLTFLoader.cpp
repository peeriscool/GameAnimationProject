...
std::vector<Mesh> LoadMeshes(cgltf_data* data) {
	...
	//loop through all nodes skip if no mesh or skin is found
	for (unsigned int i = 0; i < nodeCount; ++i) 
	{
		...
		if (node->mesh == nullptr) {
			std::cout << "Skipping node: " << (node->name ? node->name : "Unnamed") << " (no mesh)" << std::endl;
			continue; // Skip nodes without a mesh
		}
